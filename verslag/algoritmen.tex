...
\begin{gevolg}
Elke configuratie van $T$ bestaat uitsluitend uit cirkels wiens
intervallen overlappen.
\label{lemma:scanline}
\end{gevolg}
...
\begin{inv}
Elke cirkel die v\"o\"or iteratie $i$ uit $T$ verwijderd is kan
onmogelijk snijden met een cirkel die toegevoegd wordt tijdens
iteratie $i$.
\label{inv:t1}
\end{inv}
...
Nu heeft $c'$ al eens in $T$ gezeten: uit lemma \ref{lemma:scanline}
volgt dan dat onze stelling een contradictie is; immers, dan is er een
element $t \in T'$ waarin zowel $c$ als $c'$ zitten, maar dit behoort
dan tot geen enkele configuratie van $T$, gezien een cirkel die
verwijderd is niet meer toegevoegd kan worden aan $T$. Daarmee is de
invariante bewezen.
\end{proof}
...
dat van \ref{algo:naive}. Daarmee is ons correctheidsbewijs afgerond.
\end{proof}
\subsubsection{Complexiteit}
